...
# import scripts
import eye_score
import yawn_landmarks
import threshold
import fatigue_alarm
    ...
    def video_loop(self):
        ...
        while not self.stop_event.is_set():  # while the app is not closed
            ...
            if datetime.datetime.now() - last_frame_time >= datetime.timedelta(seconds=1/FRAMES_PER_SECOND):
                ...
                if eye_score.compute_average_eye_aspect_ratios(shape) < EYE_ASPECT_RATIO_THRESHOLD:
                ...
                prediction = yawn_landmarks.predict_yawn(gray_frame, model)  # [not yawn, yawn]
                ...
                if blink_counter >= threshold.blink_count_threshold(current_time, travel_duration) or \
                        yawn_counter >= threshold.yawn_count_threshold(current_time, travel_duration):
                    ...
                    if not alarm_on:  # check if the alarm is not on
                        threading.Thread(target=fatigue_alarm.sound_alarm, daemon=True).start()  # start a thread to have the alarm sound played in the background
                        ...
                        if alarm_counter == EMAIL_THRESHOLD:  # check if the alarm sounded a specific number of times - this way the email can be sent only once
                            threading.Thread(target=fatigue_alarm.send_email, args=(self.username, self.contact_name, self.contact_email), daemon=True).start()  # start a thread to send an email to emergency contact in the background
